...
class Rest {
  ...
  public function saveData( $agent_id, $citizen_mobile, $citizen_name, $application_type, $subject = NULL, $description = NULL, $email = NULL, $address = NULL, $division_bbs = NULL, $district_bbs = NULL, $upozila_bbs = NULL, $application_code, $error_handeler_code ) {
    ...
    if ( $citizen_mobile ) {
	  ...
	  $stodate=date('Y/m/d');
      $txttask_no = Rest::pick( "tbl_task", "max(task_no)", "DATE_FORMAT(`open_date`,'%Y/%m/%d')='$stodate'" );

      if ( $txttask_no < 1 ) {
        $timestamp = date( 'y/m/d' );
        $pieces = explode( "/", $timestamp );
        $txttask_no = $division_bbs . '' . $district_bbs . '' . $pieces[ 0 ] . $pieces[ 1 ] .$pieces[2]. "00001";
      } else {
        $txttask_no = Rest::pick( "tbl_task", "max(RIGHT(task_no, 5))+1", "DATE_FORMAT(`open_date`,'%Y/%m/%d')='$stodate'" );
        $txttask_nos = str_pad( $txttask_no, 5, '0', STR_PAD_LEFT );
        $timestamp = date( 'y/m/d' );
        $pieces = explode( "/", $timestamp );
        $txttask_no = $division_bbs . '' . $district_bbs . '' . $pieces[ 0 ] . '' . $pieces[ 1 ] . ''.$pieces[2].'' . $txttask_nos;
      }
