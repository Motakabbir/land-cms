...
if(isset($_REQUEST['code']))
{
	...
	$sql="SELECT code  FROM `tbl_sms_send` WHERE `number`='$contact_number' And `entry_date`='$date1' AND status=0 AND type=1";
	$res = mysql_query($sql);	
	while($rowquery=mysql_fetch_array($res))
 		{
 			extract($rowquery);
 		}
	...
	{
	echo " <span style=\"color:red\">ওয়ান টাইম পাসওয়ার্ড মেলেনি</span>";
	}
exit();
}
