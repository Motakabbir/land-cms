...
if(isset($_REQUEST['txtfromopen_date']) && $_REQUEST['txttoopen_date'] !=""){
  ...
  if($cond!=NULL){
	...
	$cond=" WHERE (DATE_FORMAT(tbl_task.open_date,'%Y-%m-%d')>='".$txtfromopen_date."' AND DATE_FORMAT(tbl_task.open_date,'%Y-%m-%d')<='".$txttoopen_date."')";}
 	...
 	if($SType ==36)
			{
				$cond.=" AND (tbl_task.service_type<12 || (tbl_task.service_type >12 && tbl_task.service_type <15) || tbl_task.service_type >22) ";
			}
// echo $cond;
 if($cond==' where 1=1'){
	// $limit='Limit 100';
...
<table class="table table-bordered table-condensed table-striped table-hover" >
	...
	$count_query   = mysql_query("SELECT COUNT(*) AS numrows FROM `tbl_task`
					...
					LEFT JOIN  tbl_taskstatus ON tbl_taskstatus.task_statusid = tbl_task.`task_status`  $cond and tbl_task.task_status=1");
	...
	$res = mysql_query("SELECT
					...
					$cond and tbl_task.task_status=1 order by tbl_taskpriority.task_priorityid desc,open_date asc LIMIT $start,$limit ");
...
<?php echo '<div class=\'col-sm-4 pl0 pr0\'>Showing '.number_format($start+1).' to ' .number_format($start+$i-1).  ' of '.number_format($Countrow).' entries </div>';		
  ...
  //   pagination($limit,$adjacent,$Countrow,$page);
	 pagination($limit,$adjacent,$Countrow,$page, 'all_complete_ticket_get.php' , '#tab42');
	 echo "</nav></div><br>";
	}
