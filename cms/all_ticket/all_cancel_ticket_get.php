...
<table class="table table-bordered table-condensed table-striped table-hover" >
	...
	$count_query   = mysql_query("SELECT COUNT(*) AS numrows FROM `tbl_task`
					...
					LEFT JOIN  tbl_taskstatus ON tbl_taskstatus.task_statusid = tbl_task.`task_status`  $cond and tbl_task.task_status=5");
	...
	$res = mysql_query("SELECT
							  ...
							  CASE 
							   WHEN  datediff(NOW(),open_date) < ".$pen."  THEN  '7'
						...
						$cond and tbl_task.task_status=5 order by tbl_taskpriority.task_priorityid desc,open_date asc LIMIT $start,$limit ");
	...
	<tr style="background: <?php echo $cl_code;?>" >
			...
			<td>
			<a href="#" class="btn btn-warning btn-xs send2" data-id="<?php echo $task_id;?>" >বিস্তারিত </a>
			</td>
		</tr>
          ...
          $('a.send2').click(function() {			  
			 ...
			 // console.log(cid);
               $('body').append($('<form/>', {
                    id: 'form',
                    method: 'POST',
                    action: 'ticket_details_cancel.php'
               }));
