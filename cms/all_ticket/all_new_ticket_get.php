...
<table class="table table-bordered table-condensed table-striped table-hover" >
	...
	$count_query   = mysql_query("SELECT COUNT(*) AS numrows FROM `tbl_task`
					...
					LEFT JOIN  tbl_taskstatus ON tbl_taskstatus.task_statusid = tbl_task.`task_status`  $cond and tbl_task.task_status=3");
	...
	$res = mysql_query("SELECT
						...
						$cond and tbl_task.task_status=3 order by tbl_taskpriority.task_priorityid desc,open_date asc LIMIT $start,$limit ");
...
<?php echo '<div class=\'col-sm-4 pl0 pr0\'>Showing '.number_format($start+1).' to ' .number_format($start+$i-1).  ' of '.number_format($Countrow).' entries </div>';		
	echo "<div class=\"col-sm-8 pl0 pr0\"><nav aria-label=\"Page navigation\">";
     pagination($limit,$adjacent,$Countrow,$page, 'all_new_ticket_get.php' , '#tab40');
	 echo "</nav></div><br>";
	}
